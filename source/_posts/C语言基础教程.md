...
> **章节概要**：储存类别(引入)；标识符、表达式与左值；作用域、链接与存储期；作用域；翻译单元和文件；链接；储存期；使块作用域变量具有静态存储期；C 的存储类别；自动变量
...
- **存储期**
  ...
  - **线程存储期**
    ...
    > 2、以**关键字**`_Thread_local_`声明一个**对象**时，每个**线程**都获得该**变量**的**私有备份**
