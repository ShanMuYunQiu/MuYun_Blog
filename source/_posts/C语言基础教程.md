...
> **章节概要**：与文件进行通信；文本模式和二进制模式；I/O 的级别；标准文件；标准 I/O；`fopen()`函数；文件指针；`getc()`和`putc()`函数；`fclose()`函数；指向标准文件的指针；简单的文件压缩程序；文件 I/O；`fprintf()`、`fscanf()`函数和`rewind()`函数；`fgets()`和`fputs()`函数；随机访问：`fseek()`和`stell()`；`fgetpos()`和`fsetpos()`函数；其他标准 I/O 函数
