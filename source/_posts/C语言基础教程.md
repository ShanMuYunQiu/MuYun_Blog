...
> **章节概要**：交互式程序；变量与常量数据；数据；位、字节、字；数据类型关键字；C 语言基本数据类型；进制打印显示；可移植类型；使用程序获得数据类型大小
...
> **章节概要**：复习函数；函数概述；函数创建与使用；函数参数与返回值；ANSI C 函数原型；旧式声明问题及解决；递归；递归演示；递归的基本原理；尾递归；递归的优缺点；编译多源代码文件的程序;使用(自建)头文件；查找地址：&运算符；更改主调函数中的变量；指针简介；指针基本概念；间接运算符：\*
...
- **基本概念**
  ...
  - 假设一个**指针变量名**是**ptr**，则可以编写`ptr = &pooh`这条语句
    ...
    > 2、**指针 ptr**和**地址 &pooh**的**区别**是，**ptr**是**变量**，**&pooh**是**常量**。或者说，**ptr**是**可修改**的**左值**，**&pooh**是**右值**  
