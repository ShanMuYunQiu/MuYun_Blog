...
> **章节概要**：储存类别(引入)；标识符、表达式与左值；作用域、链接与存储期；作用域；翻译单元和文件；链接；储存期；使块作用域变量具有静态存储期；C 的存储类别；自动变量；内层块与外层块变量同名隐藏；编译器对 C99 和 C11 的支持；寄存器变量；块作用域的静态变量；外部链接的静态变量；内部链接的静态变量；存储类别补充
