...
> **章节概要**：储存类别(引入)；标识符、表达式与左值；作用域、链接与存储期；作用域；翻译单元和文件；链接；储存期；使块作用域变量具有静态存储期；C 的存储类别；自动变量；内层块与外层块变量同名隐藏；编译器对 C99 和 C11 的支持；寄存器变量；块作用域的静态变量；外部链接的静态变量；内部链接的静态变量；存储类别补充；多文件；储存类别说明符；储存类别和函数；存储类别的选择；随机数函数和静态变量
...
- **存储类别的选择**
  ...
  - 你可能会认为**外部存储类别**很不错，把**所有变量**设置成**外部变量**，就不需要**参数**和**指针**在函数间**传递信息**了。然而，如果这样做，很可能**A函数**会**违背你的意图**，私自修改**B函数**要使用的变量。无数程序员的**经验表明**，随意使用**外部存储类别**的变量导致的**后果**会**远远超过**它带来的**便利**
