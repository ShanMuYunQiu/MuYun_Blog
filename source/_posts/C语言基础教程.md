...
> **章节概要**：翻译程序的第一步；明示常量：`#define`；明示常量简介；记号；重定义常量；在`#define`中使用参数；用宏参数创建字符串：`#`运算符；预处理器粘合剂：`##`运算符；变参宏：`...`和`__VA_ARGS__`；文件包含：`#include`；使用头文件；头文件常用形式；头文件的使用价值；其他指令；`#undef`指令；从 C 预处理器角度看已定义；条件编译
