...
- `scanf()`的**多个输入**
  ...
  - 可以通过`scanf("%d%d",&n,&m)`的格式**输入多个数据**
  - `scanf()`函数允许把**普通字符**放在**格式字符串**中，**除空格外**的字符必须与输入字符串**严格匹配**
  - 如`scanf("%d,%d",&n,&m)`，用户必须输入**两个整数**，并以**逗号分隔**
  - 除了`%c`，其他转换说明都会**自动跳过待输入值前面的所有空白**
...
> **章节概要**：再探`while`循环、`while`循环语句、迭代、关系运算符与关系表达式、真(true)与假(false)、`bool`布尔变量、`for`循环
