...
> **章节概要**：动态内存与智能指针；动态内存；智能指针；`shared_ptr`类；定义与使用智能指针；智能指针操作；`make_shared`函数；`shared_ptr`的拷贝与赋值；`shared_ptr`的销毁与释放；设计使用动态生存期资源的类；直接管理内存；`new`动态分配和初始化对象；`delete`释放内存；
