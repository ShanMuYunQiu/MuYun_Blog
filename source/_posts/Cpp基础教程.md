...
> **章节概要**：`IO`类；`IO`库类型；`IO`类型间的关系；`IO`对象；无拷贝或赋值；条件状态；查询流的状态；管理条件状态；管理输出缓冲；缓冲刷新的原因；刷新输出缓冲区；`unitbuf`操作符；`tie`关联输入和输出流；文件输入输出；`fstream`操作；`fstream`对象；`open`和`close`；文件模式；指定文件模式的限制
...
- `fstream`**操作**
  ...
  > 1、**头文件**`fstream`定义了**三个类型**：`ifstream`、`ofstream`、`fstream`，这些类型**提供的操作**与我们之前是用过的对象`cin`和`cout`**操作一样**  
