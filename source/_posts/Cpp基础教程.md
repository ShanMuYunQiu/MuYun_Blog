...
> **章节概要**：动态内存与智能指针；`shared_ptr`类(智能指针)；定义与使用智能指针；智能指针操作；`make_shared`函数；`shared_ptr`的拷贝与赋值；`shared_ptr`的销毁与释放；设计使用动态生存期资源的类；直接管理内存(动态内存指针)；`new`动态分配和初始化对象；`delete`释放内存；结合使用与异常；`shared_ptr`和`new`结合使用；智能指针和异常；自定义的释放；其他智能指针；`unique_ptr`类；`weak_ptr`类；设计核查指针类；动态数组；`new`和动态数组；智能指针管理动态数组；`allocator`类；
