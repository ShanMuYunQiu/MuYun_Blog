...
> **章节概要**：动态内存与智能指针；`shared_ptr`类(智能指针)；定义与使用智能指针；智能指针操作；`make_shared`函数；`shared_ptr`的拷贝与赋值；`shared_ptr`的销毁与释放；设计使用动态生存期资源的类；直接管理内存(动态内存指针)；`new`动态分配和初始化对象；`delete`释放内存；结合使用与异常；`shared_ptr`和`new`结合使用
...
- **new 动态分配和初始化对象**
  ...
  - `new`**的使用**
    ...
    > 2、在**自由空间**分配的内存是**无名的**，因此`new`**无法为其分配的对象命名**，而是返回一个**指向该对象的指针**。默认情况下，**动态分配**的对象是**默认初始化**的，此外我们也可以用其他一些通用的初始化方式  
