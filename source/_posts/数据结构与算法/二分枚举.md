...
- **左闭右开的写法**
  ...
  ![](https://cdn.jsdelivr.net/gh/ShanMuYunQiu/Image/blog/算法/二分查找1.png)

  ![](https://cdn.jsdelivr.net/gh/ShanMuYunQiu/Image/blog/算法/二分查找2.png)
...
- **STL 的二分查找**
  ...
  > 1、`lower_bound()`和`upper_bound()`是`STL`中的算法，用于对**有序序列**进行**二分查找**。其格式为`lower_bound(起始位置迭代器, 尾后位置迭代器, 查找值, [可选的自定义规则])`，返回值为**元素的迭代器**  
