...
- **线性求组合数**
  ...
  > 1、[求组合数-进阶](https://cdn.oj.eriktse.com/problem.php?id=1106)：有 **q 次询问**，每次需要求出`C(n, m)`，输出 q 次询问的**结果之和**，结果对 10<sup>9</sup> + 7 取模。输入 q,a,b,c(不超过 10<sup>7</sup>)，再给出`n[1]`和`m[1]`，**之后的询问**按照`(n[i], m[i]) = (n[i-1] * a + b, m[i-1] * b + a) % c`生成  
