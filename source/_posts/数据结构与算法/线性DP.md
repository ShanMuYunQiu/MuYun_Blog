...
- **单调栈二分优化**
  ...
  > 2、在**遍历求**`dp[i]`的过程中，可知：对于**点 i 在接续时**，其**左侧点 j**，如果`dp[j]`**最长**且`a[j]`**最小**，那么`dp[i]`接续到`dp[j]`后**一定为最优**，因此需要维护一个**单调栈**(单调不减的单调栈，维护栈顶为`dp[j]`最长且`a[j]`最小的点的位置)  
  ...
  > 5、对于**查找**的操作，可以使用**二分查找**，但因为其需要**操作下标**，所以**单调栈的实现**应该使用**数组实现**，不能使用`STL`的`stack`。此外，由于只需要**最长上升子序列**的**长度**，所以**单调栈的长度**就是**最长上升子序列的长度**，不再需要`dp[]`数组
