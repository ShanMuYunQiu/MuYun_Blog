...
- **SPFA 队列优化**
  ...
  > 4、由于**没有负权**，便可以使用目前速度最快的**单源最短路 Dijkstra**，以每个点为起点分别跑一次 **Dijkstra** (此处也是该程序时间复杂度瓶颈)，得出**调整后的最短路径** $d'[x][y]$。最后，将其**还原回原图的最短路径** $d[x][y] = d'[x][y] - h[x] + h[y]$ (将加上的势能差减去)，得到真实的 $d[x][y]$ **即为答案**。注意，判断**是否两点不通**时不能直接用 $\infty$ 判断(由于有负权，边被松弛时可能比 $\infty$ 小)，可以与 $\frac{\infty}{2}$ 比较(可证明松弛过的点 $d[x][y] \lt \frac{\infty}{2}$)。该算法时间复杂度为 $O(nm \log m)$
