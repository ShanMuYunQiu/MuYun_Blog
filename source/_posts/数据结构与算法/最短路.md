...
title: 最短路
author: 山暮云秋
description: 学习 Dijkstra 单源带权最短路，Floyd 全源最短路，Bellman-Ford 单源负权最短路及队列优化的 SPFA，Johnson 全源最短路
sticky: 0
readmore: true
tags:
  - Dijkstra
...
- **Dijkstra 堆优化**
  ...
  > 2、数据量变大，原先的**朴素算法**中由于有 $O(n^2)$ 会超时，可以用**优先队列**(堆)优化程序。**优先队列**中存储**点**和**起点到该点的总距离**，设定为**小根堆**，确保**堆顶元素**的**总距离最短**，因此**堆顶**即为每次 $d[i]$ **最小的点**，便不需要查找了  
