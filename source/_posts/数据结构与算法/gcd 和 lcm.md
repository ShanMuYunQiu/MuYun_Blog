...
- **拓展欧几里得算法**
  ...
  > 3、其中**迭代公式**为：$s_{i+1} = s_{i-1} - s_i \times q_i$ ，$t_{i+1} = t{i-1} - t_i \times q_i$ 。其中 $q$ 为**商**，且 $s_0 = 1, s_1 = 0, t_0 = 0, t_1 = 1$  
...
- **区间 lcm**
  ...
  > 1、[区间 lcm(链接失效)](https://cdn.oj.eriktse.com/problem.php?id=1110)：给定 $l$ 和 $r$ ($l,r \leq 40000$)，输出 $[l, r]$ **区间内所有数**的**最小公倍数**，结果对 $10^9 + 7$ 取模  
