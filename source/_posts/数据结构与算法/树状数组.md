...
- **区间修改**
  ...
  > 1、[区间修改](https://cdn.oj.eriktse.com/problem.php?id=1072)：给定一个**长度为 n 的数组 a**，**q 次操作**。操作有两种：输入`1 l r v`表示**原数组区间**`[l, r]`**累加 v**，输入`2 l r`表示**查询**`[l, r]`**区间和**并输出  
...
- **求逆序对个数**
  ...
  > 1、[求逆序对个数](https://cdn.oj.eriktse.com/problem.php?id=1017)：给定长度为 n 的数组，求**逆序对**个数。**逆序对**为满足`i < j`且`a[i] > a[j]`的**二元组**  
